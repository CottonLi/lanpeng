  ...
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['lpdatastorage:lpdbconnection:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['lpdatastorage:lpdbconnection:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['lpdatastorage:lpdbconnection:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['lpdatastorage:lpdbconnection:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="lpdbconnectionList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column label="编号" align="center" prop="id" />
      <el-table-column label="名称" align="center" prop="name" />
      <el-table-column label="类型名称" align="center" prop="dbConnectionTypeName" />
      <el-table-column label="服务器名称" align="center" prop="serverName" />
      <el-table-column label="服务器端口号" align="center" prop="serverPort" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="服务器端口号" prop="serverPort">
          <el-input v-model="form.serverPort" placeholder="请输入数据库连接---服务器端口号" />
        </el-form-item>
        <el-form-item label="数据库名称" prop="databaseName">
          <el-input v-model="form.databaseName" placeholder="请输入数据库连接---数据库名称" />
        </el-form-item>
        <el-form-item label="用户名" prop="userName">
          <el-input v-model="form.userName" placeholder="请输入数据库连接---用户名" />
        </el-form-item>
        <el-form-item label="密码" prop="userPassword">
          <el-input v-model="form.userPassword" placeholder="请输入数据库连接---密码" />
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listLpdbconnection(this.queryParams).then(response => {
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加lpdbconnection";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const id = row.id || this.ids
      getLpdbconnection(id).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改lpdbconnection";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateLpdbconnection(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addLpdbconnection(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除lpdbconnection编号为"' + ids + '"的数据项？').then(function() {
        return delLpdbconnection(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('lpdatastorage/lpdbconnection/export', {
        ...this.queryParams
      }, `lpdbconnection_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
